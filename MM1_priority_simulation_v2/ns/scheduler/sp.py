            ...
            for prio in self.priorities_list:
                if self.prio_queue_count[prio] > 0:
                    if self.zero_downstream_buffer:
                        ...
                        self.current_packet = packet
                        self.env.likelihood.start_service(packet)
                        change = self.env.likelihood.change_size(packet)
                        if change is not None:
                            self.byte_sizes[self.flow_classes(packet.flow_id)] += change
                        yield self.env.timeout(packet.size * 8.0 / self.rate)
                        self.env.likelihood.depart_service(packet)
                        ...
                        self.env.current_packet = None
                        self.env.n_served[packet.flow_id] += 1
                        
                        total_size = sum(self.byte_sizes.values())
                        self.env.check_gamma(packet.packet_info, packet.flow_id)
                        if abs(total_size) < 1e-7: # stop the cycle
                            self.env.ending()
                        self.env.last_flow = packet.flow_id
                        assert packet.packet_id == self.env.n_served[packet.flow_id], [packet.packet_id, self.env.n_served[packet.flow_id]]
