...
class DistPacketGenerator:
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            arrival_phase = self.env.phase
            interarrival = self.arrival_dist.sample(phase=arrival_phase)
            packet_info = self.env.likelihood.put_interarrival(self.flow_id, self.packets_sent, self.env.now, interarrival, arrival_phase)
            yield self.env.timeout(interarrival)
            self.env.likelihood.busy_time(self.env.now)
