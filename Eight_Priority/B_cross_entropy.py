...
def CE_update(lam, mu, num_type, port_rate, threshold, test_flow, cycle_len, cpu_nums, n_cycles, policy, rho):
    start = time.time()
    # simulate
...
def CE_main(num_type, lam_ref, mu_ref, cycle_length, port_rate, rho, test_flow, cpu_nums, n_cycles, policy, tile, num_runs, test_cycles, file_name):
    ...
    while CONTINUE:
        ...
        # update parameters
        lam, mu, gamma, n_packet, mean, halfCI, time_iter = CE_update(lam.copy(), mu.copy(), num_type, port_rate, threshold, test_flow, cycle_len, cpu_nums, n_cycles, policy, rho)
        info.append(gamma)
