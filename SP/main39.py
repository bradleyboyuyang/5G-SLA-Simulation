...
parser.add_argument('--n_cycles', type=int, default=10000, help='')
...
parser.add_argument('--CoE_phase', type=int, default=1)
...
if __name__ == '__main__':
    random = np.random.RandomState(3425)
    # num_type = 4
    ...
    if args.test_case == 0:
        ...
        if args.CoE_phase == 0:
            ...
            if args.test_flow == 0:
                lam_tilt = [ 0.33035071791434534,0.22431286291808383]
                mu_tilt = [0.23375617426503906,0.23847807510935579]
            elif args.test_flow == 1:
        ...
        if args.test_flow == 0:
            ...
            args.gamma_ref =  14.463523349010309
        if args.CoE_phase == 0:
            lam_tilt = lam.copy()
            mu_tilt = mu.copy()
        elif args.CoE_phase == 1:
            lam_tilt =  [0.43166247903554, 0.86332495807108]   
            mu_tilt = [0.92664991614216, 0.46332495807108]
        elif args.CoE_phase == 2:
            if args.test_flow == 0:
                lam_tilt = [0.334178,  0.623941]
                mu_tilt = [1.157432,  0.253821]
            elif args.test_flow == 1:
                lam_tilt =  [0.217185 , 0.895761]   
                mu_tilt =  [1.12159,  0.386477]
    ...
    for idx in range(args.unbiased_check):
        ...
        import csv 
        with open(f'./results/unbiased_check/Bcase{args.test_case}_phase{args.CoE_phase}_K{num_type}_flow{args.test_flow}_n{args.n_cycles}_quantile.csv', "a") as f:
            writer = csv.writer(f)
            writer.writerows([L])