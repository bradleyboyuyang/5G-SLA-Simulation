...
from utils import continue_run, cycle_statistics, parameter, quantile_CI, quantile_var, sample_quantile
...
parser.add_argument('-t','--test_flow', type=int, default=5, help='the number of cpus')
...
parser.add_argument('--batch_quantile_flag', type=int, default=500)
...
parser.add_argument('--RE', type=float, default=0.001)
parser.add_argument('--increment', type=float, default=0)
parser.add_argument('--file_name', type=str, default='class859')
...
def main(lam_original, mu_original, tile, random, rho, batch_quantile_flag):
    ...
    lam_de, mu_de = arrival_rates.copy(), service_rates.copy()
    def run_denominator(cycles):
        return static_priority_sim(num_type, lam_de, mu_de, args.test_flow, 100, 8, ExponDistri, args.cpu_nums, cycles, time.time(), True, {"cycle_len":True})['cycle_len']
    ...
    lam, mu, gamma_l, gamma_u, gamma = CE_up(num_type, arrival_rates, service_rates, cycle_len, 8, rho, args.test_flow, args.cpu_nums, args.CE_cycles, tile, args.stable_iter, None, args.file_name, gamma_ref=args.gamma_ref)
    ...
    def run_nominator(cycles):
        return static_priority_sim(num_type, arrival_rates, service_rates, args.test_flow, args.gamma_ref, 8, ExponDistri, args.cpu_nums, cycles, time.time(), True, {'save_response':True})['response']

    Q2_total = sample_quantile(result['response'], cycle_len, tile)
    print(f'sample {tile} quantile is {Q2_total}')
    Q2_total, section_std, batch_mean, batch_std, L_batch = quantile_CI(cycle_len, result['response'], batch_quantile_flag, tile, Q2_total, random, True)
    L_cycle = cycle_len
    L_nominator = result['response']
    while section_std / Q2_total > args.RE:
        n_cycle_add = (section_std / Q2_total / args.RE)**2
        print(f"{n_cycle_add}. exist {len(L_cycle)}")
        L_cycle, L_nominator, Q2_total, section_std, L_batch = continue_run(run_denominator, run_nominator, L_cycle, L_nominator,L_batch, int((n_cycle_add-1+args.increment)*len(L_cycle)), batch_quantile_flag, tile, random)
