...
def main(n):
    ...
    while True:
        ...
        if (len(estimate1) > 10000) & (len(estimate2) > 10000): 
            break

    estimate_wat1, estimate_std1 = np.mean(estimate1), np.std(estimate1)
    estimate_wat2, estimate_std2 = np.mean(estimate2), np.std(estimate2)
