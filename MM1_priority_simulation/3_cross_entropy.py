...
def cycle_sta1(cycle_num, cycle_sum):
    N = len(cycle_num)
...
def main(lam, mu, port_rate, threshold, args, CoE_p):
    ...
    result = static_priority_sim(num_type, lam, mu, threshold, args.test_flow, port_rate, ExponDistri, args.cpu_nums, n_cycles, start, p=CoE_p, policy=args.policy)
    ...
    mean, halfCI = cycle_sta1(cycle_num, cycle_sum)
    ...
    for i, gamma in enumerate(threshold[0]):
        ...
        if 0 == args.test_flow and gamma == threshold[0][-1]:
            RE = halfCI/1.96/mean
        mean, halfCI = cycle_sta1(cycle_num, cycle_sum_prob)
