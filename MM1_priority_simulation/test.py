...
parser.add_argument('-n', '--n_cycles', type=int, default=1000, help='the size of the training dataset')
...
def main(lam, mu, port_rate, threshold, args, mix_p, num_type):
    ...
    # calculate type 1, first list in L
    flow_id = 0
    ...
    # calculate type 2, second list in L
    flow_id = 1
    cycle_num = [cycle_num[flow_id] for cycle_num in cycle_nums]
    cycle_sum = [cycle_sum[flow_id] for cycle_sum in cycle_sums]
    ...
    # import csv 
    # with open("result/1_run_para_test.csv", "a", newline='') as f:
    #     writer = csv.writer(f)
