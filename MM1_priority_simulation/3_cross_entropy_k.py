...
def main(lam, mu, port_rate, threshold, args, CoE_p, num_type, start):
    ...
    result = static_priority_sim(num_type, lam, mu, threshold, args.test_flow, port_rate, ExponDistri, args.cpu_nums, args.n_cycles, start_local, p=CoE_p, policy=args.policy)
    cycle_nums, cycle_sums, cycle_sum_probs, reasons, nums, CE, waits = result
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t','--test_flow', type=int, default=3, help='which flow to switch measure')
    parser.add_argument('-T','--threshold', type=float, default=3, help='threshold')
